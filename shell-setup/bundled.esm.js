// deno:https://jsr.io/@david/which/0.4.1/mod.ts
...
async function tryStat(path) {
  try {
    ...
    if (error instanceof Deno.errors.NotFound || error instanceof Deno.errors.PermissionDenied && (await Deno.permissions.query({
      name: "read",
      path
    })).state == "granted") {
      return;
...
function makeWrapper() {
  ...
  for (const keyString in _environmentImpl) {
    ...
    if (typeof _environmentImpl[key] === "function") {
      wrapperEnv[key] = function(...args) {
        return _environmentImpl[key](...args);
      };
    }
  }
...
// deno:https://jsr.io/@std/path/1.1.3/_common/basename.ts
...
// deno:https://jsr.io/@std/path/1.1.3/posix/basename.ts
...
// deno:https://jsr.io/@std/path/1.1.3/windows/basename.ts
...
// deno:https://jsr.io/@std/path/1.1.3/basename.ts
...
// deno:https://jsr.io/@nathanwhit/promptly/0.1.2/mod.ts
var encoder = new TextEncoder();
var decoder = new TextDecoder();
var Key = /* @__PURE__ */ function(Key2) {
  Key2[Key2["Up"] = 0] = "Up";
  Key2[Key2["Down"] = 1] = "Down";
  Key2[Key2["Left"] = 2] = "Left";
  Key2[Key2["Right"] = 3] = "Right";
  Key2[Key2["Enter"] = 4] = "Enter";
  Key2[Key2["Space"] = 5] = "Space";
  Key2[Key2["Backspace"] = 6] = "Backspace";
  return Key2;
}(Key || {});
...
var CursorDir = /* @__PURE__ */ function(CursorDir2) {
  CursorDir2[CursorDir2["Up"] = 0] = "Up";
  CursorDir2[CursorDir2["Down"] = 1] = "Down";
  CursorDir2[CursorDir2["Left"] = 2] = "Left";
  CursorDir2[CursorDir2["Right"] = 3] = "Right";
  CursorDir2[CursorDir2["Column"] = 4] = "Column";
  return CursorDir2;
}(CursorDir || {});
var charCodes = (...cs) => {
...
function eraseToEnd(writer) {
  writeAll(writer, new Uint8Array([
    27,
    91,
    codes[0],
    codes.K
...
function writeLines(writer, lines) {
  ...
  for (const [i, line] of lines.entries()) {
    ...
    writer.writeSync(encoder.encode(line + suffix));
  }
  moveCursor(writer, CursorDir.Column);
}
...
function maybeMultiSelect(options) {
  ...
  const { selected = "[x]", unselected = "[ ]", pointer = ">", listBullet = "-", messageStyle = (s) => bold(blue(s)) } = options.styling ?? {};
  const style = {
    selected,
    unselected,
    pointer,
  ...
  if (state.hasCompleted) {
    if (state.items.some((i) => i.selected)) {
      for (const item of state.items) {
        if (item.selected) {
          items.push(`${" ".repeat(style.pointer.length + style.selected.length - style.listBullet.length - 2)}${style.listBullet} ${item.text}`);
        }
      }
    } else {
    ...
    for (const [i, item] of state.items.entries()) {
      ...
      items.push(`${prefix}${item.selected ? style.selected : style.unselected} ${item.text}`);
    }
  }
  return items;
}
...
// deno:https://jsr.io/@std/cli/1.0.24/parse_args.ts
...
// src/shell.ts
var { isExistingFile, writeTextFile, homeDir, findCmd, runCmd, getEnv, pathExists } = environment;
var ShellScript = class {
  ...
  async write(denoInstallDir) {
    ...
    try {
      ...
      throw withContext(`Failed to write ${this.name} file to ${envFilePath}`, error);
    }
  }
};
var shEnvScript = (installDir) => new ShellScript("env", `#!/bin/sh
...
var Zsh = class {
  ...
  async rcfiles() {
    ...
    return [
      zshDotDir,
      homeDir
    ].map((dir) => dir ? join3(dir, ".zshrc") : void 0).filter((dir) => dir !== void 0);
  }
  async rcsToUpdate() {
    let out = await filterAsync(await this.rcfiles(), isExistingFile);
    if (out.length === 0) {
...
if [[ ":$FPATH:" != *":${completionDir}:"* ]]; then export FPATH="${completionDir}:$FPATH"; fi`;
    ...
    if ((await filterAsync([
      ...
      ".oh_my_zsh",
      ".zprezto"
    ], (f) => pathExists(join3(zshDotDir, f)))).length == 0) {
...
var RcBackups = class {
  ...
  constructor(backupDir) {
    ...
    this.backedUp = /* @__PURE__ */ new Set();
  }
...
// src/main.ts
var { readTextFile, runCmd: runCmd2, writeTextFile: writeTextFile2 } = environment;
async function writeCompletionFiles(availableShells) {
  ...
  for (const shell of availableShells) {
    ...
    try {
      ...
      const output = await runCmd2(Deno.execPath(), [
        "completions",
        shell.name
      ]);
      if (!output.success) {
        throw new Error(`deno completions subcommand failed, stderr was: ${decoder2.decode(output.stderr)}`);
      }
      ...
      if (currentContents !== completionFileContents) {
        if (currentContents !== null) {
          warn(`an existing completion file for deno already exists at ${completionFilePath}, but is out of date. overwriting with new contents`);
        }
...
async function setupShells(installDir, backupDir, opts) {
  const { skipPrompts, noModifyPath } = opts;
  ...
  if (completionsToSetup.length > 0) {
    ...
    await writeCompletionRcCommands(completionsToSetup.filter((_s, i) => results[i] !== "fail"), backups);
  }
}
function printHelp() {
  console.log(`
...
async function main() {
  if (Deno.args.length === 0) {
    throw new Error("Expected the deno install directory as the first argument");
  }
  ...
  try {
    ...
    warn(`Failed to configure your shell environments, you may need to manually add deno to your PATH environment variable.
